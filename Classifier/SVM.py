...
import argparse
##       pos_neg_152304_fusion_train         pos_neg_bigmodel_fusion_train456      pos_neg_coding_bigmodel_fusion_train456
...
yscore_sum.to_csv('../output/yscore_SVM_cross.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('../output/ytest_SVM_cross.csv')

######在这里绘制曲线
# print(ytest.shape)
# print(yscore.shape)
# utils.draw_roc(ytest,yscore)



fpr, tpr, _ = roc_curve(ytest[:,0], yscore[:,0])###只要有这两个数据就可以绘制ROC曲线
auc_score=np.mean(scores, axis=0)[7]
lw=2  ###表示线条的粗细
plt.plot(fpr, tpr, color='darkorange', lw=lw, label='SVM_cross ROC (area = %0.2f%%)' % auc_score)
plt.plot(fpr, tpr, color='black', lw=lw, label='SVMMMMM_cross ROC (area = %0.2f%%)' % auc_score)
plt.plot([0, 1], [0, 1], color='navy', lw=lw, linestyle='--')  ###就是这个虚线的画法
plt.xlim([0.0, 1.05])  ###横坐标
plt.ylim([0.0, 1.05])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver operating characteristic')
plt.legend(loc="lower right")###位置
#plt.show()
data_csv = pd.DataFrame(data=result)
data_csv.to_csv('../output/result_SVM_cross.csv')

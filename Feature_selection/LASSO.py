...
parser.add_argument("--out_to_csv", type=str, default="../output/pos_neg_coding_bigmodel_fusion_train456_LASSO.csv", help="the output file")
...
result1=np.mean(scores,axis=0)###最后一个是平均值
H1=result1.tolist()
sepscores.append(H1)
result=sepscores
row=yscore.shape[0]
...
yscore_sum.to_csv('../output/yscore_LASSO_cross.csv')
ytest=ytest[np.array(range(1,row)),:]
ytest_sum = pd.DataFrame(data=ytest)
ytest_sum.to_csv('../output/ytest_LASSO_cross.csv')
...
data_csv.to_csv('../output/result_LASSO_cross.csv')
